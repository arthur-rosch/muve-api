...
class AddInfoFirstAccessUseCase {
    ...
    execute(_a) {
        return __awaiter(this, arguments, void 0, function* ({ userId, accountType, memberArea, videoHosting, }) {
            const user = yield this.usersRepository.findById(userId);
            if (!user) {
                throw new erros_1.NotFoundErros('User');
            }
            const newUser = yield this.usersRepository.update(user.id, Object.assign(Object.assign({}, user), { accountType,
                memberArea,
                videoHosting }));
            return {
                user: newUser,
