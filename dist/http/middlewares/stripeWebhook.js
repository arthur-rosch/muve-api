...
async function handleStripeWebhook(request, reply) {
    ...
    switch (event.type) {
        case 'checkout.session.expired':
            await (0, checkout_expired_1.checkoutExpired)(request, reply, event.data.object);
            break;
        case 'checkout.session.completed':
            await (0, checkout_completed_1.checkoutCompleted)(request, reply, event.data.object);
            break;
        case 'customer.subscription.deleted':
            await (0, subscription_deleted_1.subscriptionDeleted)(reply, request, event.data.object);
            break;
        case 'customer.subscription.updated':
            await (0, subscription_update_1.subscriptionUpdate)(reply, request, event.data.object);
            break;
        case 'invoice.payment_failed':
            await (0, invoice_payment_failed_1.invoicePaymentFailed)(reply, request, event.data.object);
            break;
        case 'invoice.payment_succeeded':
            await (0, invoice_payment_succeeded_1.invoicePaymentSucceeded)(reply, request, event.data.object);
            break;
        default:
